...
void test_pybind(string graph_type = "erdos_renyi",
  ...
  const bool concat_edges = true, const bool query_at_end = false,
  ...
  const int max_edges = 512) {
    ...
    if ( graph_type == "erdos_renyi" ) {
        ...
        d = erdos_renyi_n(num_nodes, num_nodes + 5, -1.0, 75, 125,
            10, 3, true,
            -1, 2, false, false,
            is_casual, shuffle_edges, shuffle_nodes, min_vocab, max_vocab, batch_size, max_edges, 1000,
...
int main(){
  ...
  auto task_end_marker = dictionary["."];
    ...
    cout << "special tokens are:"
        ...
        "\n\tquery_end_marker: " << query_end_marker <<
        "\n\ttask_start_marker: " << task_start_marker <<
        "\n\ttask_end_marker: " << task_end_marker <<
        "\n\tthinking_token: " << thinking_token << endl;
    ...
    auto t1 = time_before();
    test_pybind("erdos_renyi_n", num_nodes, batch_size, is_casual, shuffle_edges, shuffle_nodes, min_vocab, max_vocab, concat_edges ,query_at_end, is_flat_model, for_plotting);
